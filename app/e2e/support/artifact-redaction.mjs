export function redactArtifact(value) {
  ...
  const content = value
    ...
    .replace(/(__TWD_DATA_TOKEN__\s*=\s*(?:\\+)?["'])(?:v1\.\d+\.)?[a-f0-9]{64}\b/gi, (_match, prefix) => `${prefix}${replacement("DASH_TOKEN")}`)
    .replace(/(Bearer\s+)(?:v1\.\d+\.)?[a-f0-9]{64}\b/gi, (_match, prefix) => `${prefix}${replacement("DASH_TOKEN")}`)
