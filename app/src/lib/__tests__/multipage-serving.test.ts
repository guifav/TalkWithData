...
describe("referer-based auth for sub-resources", () => {
  ...
  it("accepts referer from same dashboard view", () => {
    expect(
      isValidReferer(
        "https://app.example.com/api/dashboards/abc123/view",
        "abc123"
      )
    ).toBe(true);
  });
  ...
  it("accepts referer from same dashboard with embed token", () => {
    expect(
      isValidReferer(
        "https://app.example.com/api/dashboards/abc123/view?embed_token=xxx",
        "abc123"
      )
    ).toBe(true);
  });
  ...
  it("rejects referer from different dashboard", () => {
    expect(
      isValidReferer(
        "https://app.example.com/api/dashboards/other-id/view",
        "abc123"
      )
    ).toBe(false);
  });
