...
• **Fluxo:** Descreva o dashboard que precisa then a IA consulta os dados via MCP then gera um dashboard HTML interativo then você pode refinar com instruções adicionais.
...
• **Flow:** Describe the dashboard you need then the AI queries data via MCP then generates an interactive HTML dashboard then you can refine with additional instructions.
...
• **Flujo:** Describe el dashboard que necesitas then la IA consulta datos vía MCP then genera un dashboard HTML interactivo then puedes refinar con instrucciones adicionales.
